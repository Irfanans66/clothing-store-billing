...
import {
  ...
  Typography, Tag, Space, Modal, Form, message, Tabs, Descriptions,
} from 'antd'
...
export default function SuperAdmin() {
  ...
  const storeColumns = [
    { title: 'Store Code', dataIndex: 'store_code', key: 'store_code', width: 110,
    ...
    { title: 'Signup', dataIndex: 'created_at', key: 'created_at', width: 110,
  ...
  const ticketColumns = [
    ...
    { title: 'Store', key: 'store', width: 130,
      render: (_, r) => (
        <div>
    ...
    { title: 'Action', key: 'action', width: 80,
      render: (_, r) => (
        <Button
          ...
          onClick={() => { setReplyModal(r); replyForm.setFieldsValue({ status: r.status, admin_reply: r.admin_reply || '' }) }}
        >
          Reply
        </Button>
      ),
  ...
  return (
    <div>
              ...
              <>
                ...
                <Card style={{ borderRadius: 12 }}>
                  ...
                  <Table
                    ...
                    expandable={{
                      expandedRowRender: (s) => (
                        <Descriptions size="small" bordered column={2} style={{ margin: '8px 0' }}>
                          <Descriptions.Item label="Address" span={2}>{s.address || '—'}</Descriptions.Item>
                          <Descriptions.Item label="Email">{s.email || '—'}</Descriptions.Item>
                          <Descriptions.Item label="Phone">{s.phone || '—'}</Descriptions.Item>
                          <Descriptions.Item label="Customers">{s.customers}</Descriptions.Item>
                          <Descriptions.Item label="Products">{s.products}</Descriptions.Item>
                          <Descriptions.Item label="Total Sales">₹{Math.round(s.revenue || 0).toLocaleString()}</Descriptions.Item>
                          <Descriptions.Item label="Signup Date">{s.created_at ? new Date(s.created_at).toLocaleString() : '—'}</Descriptions.Item>
                          <Descriptions.Item label="Notes" span={2}>{s.notes || '—'}</Descriptions.Item>
                        </Descriptions>
                      ),
                    }}
                  />
      ...
      <Modal
        ...
        open={!!replyModal} onCancel={() => { setReplyModal(null); replyForm.resetFields() }} footer={null}
        width={560}
      >
        {replyModal && (
          <div style={{ marginBottom: 16, background: '#f5f5f5', borderRadius: 8, padding: '10px 14px' }}>
            <Text type="secondary" style={{ fontSize: 12 }}>From {replyModal.username} ({replyModal.store_name}):</Text>
